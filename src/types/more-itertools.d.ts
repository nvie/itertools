...
export function* heads<T>(...iters: Iterable<T>[]): Iterable<T[]>;
export function take<T>(n: number, iterable: Iterable<T>): T[];
export function* uniqueEverseen<T>(iterable: Iterable<T>, keyFn?: (T) => Primitive): Iterable<T>;
export function* uniqueJustseen<T>(iterable: Iterable<T>, keyFn?: (T) => Primitive): Iterable<T>;
